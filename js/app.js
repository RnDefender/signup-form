...
*/
    ...
    let formValidate = {
        getErrors(form) {
            ...
            if (formRequiredItems.length) formRequiredItems.forEach((formRequiredItem => {
                if ((formRequiredItem.offsetParent !== null || formRequiredItem.tagName === "SELECT") && !formRequiredItem.disabled) if (formRequiredItem.dataset.required === "email") {
                    formRequiredItem.value = formRequiredItem.value.replace(" ", "");
                    if (this.emailTest(formRequiredItem)) {
                        this.addError(formRequiredItem);
                        error++;
                    } else this.removeError(formRequiredItem);
                } else if (formRequiredItem.dataset.required === "password") error += this.validatePassword(formRequiredItem); else error += this.validateInput(formRequiredItem);
            }));
            return error;
        },
        validateInput(formRequiredItem) {
            let error = 0;
            if (formRequiredItem.type === "checkbox" && !formRequiredItem.checked) {
                this.addError(formRequiredItem);
                error++;
        ...
        validatePassword(formRequiredItem) {
            let error = 0;
            let password = formRequiredItem.value;
            var uppercaseRegex = /[A-Z]/;
            var lowercaseRegex = /[a-z]/;
            var digitRegex = /[0-9]/;
            if (password.length < 8 || !uppercaseRegex.test(password) || !lowercaseRegex.test(password) || !digitRegex.test(password)) {
                this.addError(formRequiredItem);
                error++;
            } else this.removeError(formRequiredItem);
            return error;
    ...
    function formSubmit(options = {
        ...
        async function formSubmitAction(form, e) {
            ...
            if (error === 0) {
                ...
                e.preventDefault();
                gsapWithCSS.to(".form-signup__button", {
                    x: -10,
                    duration: .07,
                    repeat: 5,
                    yoyo: true
    ...
    const formFields = gsapWithCSS.utils.toArray(".form-signup__field");
    const formTimeline = gsapWithCSS.timeline();
